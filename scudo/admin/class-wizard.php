...
class Scudo_Wizard {
    ...
    public static function ajax_finish(): void {
        ...
        $cookie_page_id = wp_insert_post( [
            'post_title'   => __( 'Cookie Policy', 'scudo' ),
        ...
        if ( ! is_wp_error( $cookie_page_id ) ) {
            ...
            $created[] = [ 'title' => __( 'Cookie Policy', 'scudo' ), 'url' => get_permalink( $cookie_page_id ) ];
        }
        ...
        if ( class_exists( 'Scudo_Privacy_Policy' ) ) {
            ...
            if ( ! is_wp_error( $privacy_page_id ) ) {
                ...
                $created[] = [ 'title' => __( 'Privacy Policy', 'scudo' ), 'url' => get_permalink( $privacy_page_id ) ];
            }
        }

        // 3. Crea pagina Esercizio Diritti
        $rights_page_id = wp_insert_post( [
            'post_title'   => __( 'Esercita i tuoi diritti', 'scudo' ),
            'post_content' => '<p>' . __( 'Ai sensi degli Artt. 15-22 del GDPR, puoi esercitare i tuoi diritti compilando il modulo seguente. Risponderemo entro 30 giorni.', 'scudo' ) . '</p>' . "\n\n" . '[scudo_rights_form]',
            'post_status'  => 'publish',
            'post_type'    => 'page',
        ] );
        if ( ! is_wp_error( $rights_page_id ) ) {
            $created[] = [ 'title' => __( 'Esercita i tuoi diritti', 'scudo' ), 'url' => get_permalink( $rights_page_id ) ];
        }
        ...
        wp_send_json_success( [
            ...
            'message' => __( 'Scudo è attivo! Il tuo sito è ora conforme al GDPR.', 'scudo' ),
        ] );
    }
    ...
    public static function render(): void {
        ...
        <style>
            ...
            @media(max-width:600px) {
                .scudo-wiz { margin:20px 10px; }
                .scudo-wiz-card { padding:16px; }
                .scudo-wiz-themes { flex-direction:column; }
                .scudo-wiz-theme { width:100%; }
            }
        </style>

        <div class="scudo-wiz">
            <h1><?php esc_html_e( 'Configura Scudo', 'scudo' ); ?></h1>
            <p class="subtitle"><?php esc_html_e( 'Metti il tuo sito a norma GDPR in 2 minuti. I dati si salvano automaticamente.', 'scudo' ); ?></p>
            ...
            <div class="scudo-wiz-step active" data-step="1">
                <div class="scudo-wiz-card">
                    <h2><?php esc_html_e( 'Chi sei?', 'scudo' ); ?></h2>
                    <p class="desc"><?php esc_html_e( 'I dati del titolare del trattamento. Appariranno nella Privacy Policy come richiesto dall\'Art. 13 del GDPR.', 'scudo' ); ?></p>

                    <div class="scudo-wiz-field">
                        <label><?php esc_html_e( 'Ragione sociale o nome completo *', 'scudo' ); ?></label>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        <div class="scudo-wiz-field">
                            <label><?php esc_html_e( 'Email *', 'scudo' ); ?></label>
                            <input type="email" data-key="controller_email" value="<?php echo esc_attr( $privacy_data['controller_email'] ); ?>">
                        </div>
                        <div class="scudo-wiz-field">
                            <label><?php esc_html_e( 'Telefono', 'scudo' ); ?></label>
                            <input type="tel" data-key="controller_phone" value="<?php echo esc_attr( $privacy_data['controller_phone'] ); ?>">
                        </div>
                    </div>
                    <div class="scudo-wiz-field">
                        <label><?php esc_html_e( 'PEC (opzionale)', 'scudo' ); ?></label>
                        <input type="email" data-key="controller_pec" value="<?php echo esc_attr( $privacy_data['controller_pec'] ); ?>">
                    </div>
                </div>

                <div class="scudo-wiz-card">
                    <h2><?php esc_html_e( 'Hai un DPO?', 'scudo' ); ?></h2>
                    <p class="desc"><?php esc_html_e( 'Il Responsabile della Protezione dei Dati è obbligatorio per enti pubblici e organizzazioni che trattano dati su larga scala. Per la maggior parte dei siti WordPress non serve.', 'scudo' ); ?></p>
                    <label class="scudo-wiz-check" data-key="dpo_enabled" data-value="toggle">
                        ...
                        <span class="info"><strong><?php esc_html_e( 'Ho nominato un DPO', 'scudo' ); ?></strong></span>
                    </label>
                    <div id="wiz-dpo-fields" style="<?php echo $privacy_data['dpo_enabled'] ? '' : 'display:none;'; ?>margin-top:12px;">
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                            <div class="scudo-wiz-field"><label><?php esc_html_e( 'Nome DPO', 'scudo' ); ?></label><input type="text" data-key="dpo_name" value="<?php echo esc_attr( $privacy_data['dpo_name'] ); ?>"></div>
                            <div class="scudo-wiz-field"><label><?php esc_html_e( 'Email DPO', 'scudo' ); ?></label><input type="email" data-key="dpo_email" value="<?php echo esc_attr( $privacy_data['dpo_email'] ); ?>"></div>
                        </div>
                    </div>
                </div>
                <span class="scudo-wiz-saved"><?php esc_html_e( 'Salvato', 'scudo' ); ?></span>
            </div>

            <!-- ═══ STEP 2: Cosa fa il tuo sito ═══ -->
            <div class="scudo-wiz-step" data-step="2">
                <div class="scudo-wiz-card">
                    <h2><?php esc_html_e( 'Cosa fa il tuo sito?', 'scudo' ); ?></h2>
                    <p class="desc"><?php esc_html_e( 'Seleziona le attività che il tuo sito svolge. Scudo ha già rilevato alcune funzionalità.', 'scudo' ); ?></p>
                    <div id="wiz-purposes">
                    <?php
                    $purposes = [
                        'site_functionality' => [ __( 'Sito web funzionante', 'scudo' ), __( 'Cookie tecnici per il funzionamento base', 'scudo' ), true ],
                        'contact_forms'      => [ __( 'Moduli di contatto', 'scudo' ), __( 'Raccoglie nome, email e messaggi dai visitatori', 'scudo' ), false ],
                        'analytics'          => [ __( 'Statistiche e analisi', 'scudo' ), __( 'Google Analytics, Hotjar, Clarity o simili', 'scudo' ), false ],
                        'marketing'          => [ __( 'Marketing e pubblicità', 'scudo' ), __( 'Google Ads, Facebook Pixel, remarketing', 'scudo' ), false ],
                        'newsletter'         => [ __( 'Newsletter', 'scudo' ), __( 'Mailchimp, MailPoet o altro servizio di email marketing', 'scudo' ), false ],
                        'ecommerce'          => [ __( 'E-commerce', 'scudo' ), __( 'Vendita online con WooCommerce o simili', 'scudo' ), false ],
                        'user_accounts'      => [ __( 'Account utente', 'scudo' ), __( 'Registrazione e login degli utenti', 'scudo' ), false ],
                        'comments'           => [ __( 'Commenti', 'scudo' ), __( 'I visitatori possono commentare gli articoli', 'scudo' ), false ],
                    ];
                    ...
                    <label class="scudo-wiz-check <?php echo $checked ? 'checked' : ''; ?>" data-purpose="<?php echo esc_attr( $key ); ?>">
                        ...
                        <?php if ( $forced ) : ?><span class="badge"><?php esc_html_e( 'sempre attivo', 'scudo' ); ?></span><?php endif; ?>
                    </label>
                    <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="scudo-wiz-step" data-step="3">
                <div class="scudo-wiz-card">
                    <h2><?php esc_html_e( 'Quali servizi esterni usi?', 'scudo' ); ?></h2>
                    <p class="desc"><?php esc_html_e( 'Scudo ha scansionato il tuo sito e ha rilevato alcuni servizi. Verifica e aggiungi quelli mancanti.', 'scudo' ); ?></p>
                    <div id="wiz-services">
                    <?php
                    $services = [
                        'google_analytics' => [ 'Google Analytics', __( 'Statistiche di traffico e comportamento utenti', 'scudo' ) ],
                        'google_ads'       => [ 'Google Ads', __( 'Pubblicità e remarketing Google', 'scudo' ) ],
                        'google_maps'      => [ 'Google Maps', __( 'Mappe interattive nelle pagine', 'scudo' ) ],
                        'google_fonts'     => [ 'Google Fonts', __( 'Font tipografici caricati da Google', 'scudo' ) ],
                        'youtube'          => [ 'YouTube', __( 'Video incorporati nelle pagine', 'scudo' ) ],
                        'facebook_pixel'   => [ 'Facebook / Meta Pixel', __( 'Tracciamento per pubblicità Facebook e Instagram', 'scudo' ) ],
                        'mailchimp'        => [ 'Mailchimp', __( 'Servizio di email marketing e newsletter', 'scudo' ) ],
                        'stripe'           => [ 'Stripe', __( 'Elaborazione pagamenti online', 'scudo' ) ],
                        'paypal'           => [ 'PayPal', __( 'Elaborazione pagamenti online', 'scudo' ) ],
                        'cloudflare'       => [ 'Cloudflare', __( 'CDN e protezione del sito', 'scudo' ) ],
                        'hotjar'           => [ 'Hotjar', __( 'Heatmap e registrazioni sessioni utente', 'scudo' ) ],
                    ];
            ...
            <div class="scudo-wiz-step" data-step="4">
                <div class="scudo-wiz-card">
                    <h2><?php esc_html_e( 'Come vuoi che appaia il banner?', 'scudo' ); ?></h2>
                    <p class="desc"><?php esc_html_e( 'Scegli lo stile. Potrai cambiarlo in qualsiasi momento dalle impostazioni.', 'scudo' ); ?></p>
                    <div class="scudo-wiz-themes">
                        <?php
                        $theme_options = [
                            'dark'  => [ __( 'Scuro', 'scudo' ), '#1a1a2e', '#fff', '#fff', '#1a1a2e' ],
                            'light' => [ __( 'Chiaro', 'scudo' ), '#ffffff', '#1a1a2e', '#1a1a2e', '#fff' ],
                        ];
            ...
            <div class="scudo-wiz-step" data-step="5">
                <div class="scudo-wiz-card">
                    <div class="scudo-wiz-result" id="wiz-result-pending">
                        <h2><?php esc_html_e( 'Tutto pronto!', 'scudo' ); ?></h2>
                        <p><?php esc_html_e( 'Scudo creerà automaticamente:', 'scudo' ); ?></p>
                        <ul style="text-align:left;max-width:400px;margin:16px auto;font-size:14px;line-height:2;">
                            <li><?php esc_html_e( 'Pagina Cookie Policy con la tabella dei cookie', 'scudo' ); ?></li>
                            <li><?php esc_html_e( 'Pagina Privacy Policy con i tuoi dati', 'scudo' ); ?></li>
                            <li><?php esc_html_e( 'Pagina per l\'esercizio dei diritti GDPR', 'scudo' ); ?></li>
                            <li><?php esc_html_e( 'Banner dei cookie con il tema scelto', 'scudo' ); ?></li>
                            <li><?php esc_html_e( 'Registro dei consensi attivo', 'scudo' ); ?></li>
                        </ul>
                        <button type="button" class="btn btn-finish" id="wiz-finish-btn"><?php esc_html_e( 'Attiva Scudo', 'scudo' ); ?></button>
                    </div>
            ...
            <div class="scudo-wiz-nav">
                <button type="button" class="btn btn-back" id="wiz-back" style="visibility:hidden;"><?php esc_html_e( 'Indietro', 'scudo' ); ?></button>
                <button type="button" class="btn btn-next" id="wiz-next"><?php esc_html_e( 'Avanti', 'scudo' ); ?></button>
            </div>
        </div>
        ...
        (function(){
            ...
            document.getElementById('wiz-finish-btn').addEventListener('click', function(){
                this.disabled = true;
                this.textContent = '<?php echo esc_js( __( 'Configurazione in corso...', 'scudo' ) ); ?>';
                var fd = new FormData();
                    ...
                    .then(function(data){
                        ...
                        Object.keys(d.purposes||{}).forEach(function(k){
                            ...
                            if(el && d.purposes[k] && !el.querySelector('input').checked){
                                ...
                                badge.textContent = '<?php echo esc_js( __( 'rilevato', 'scudo' ) ); ?>';
                                el.appendChild(badge);
                            }
                        });
                        // Spunta i servizi rilevati
