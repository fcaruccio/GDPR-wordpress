...
class Scudo_Privacy_Policy {
    ...
    public static function render_wizard(): void {
        ...
        ?>
        <hr>
        <h2 class="title"><?php esc_html_e( 'Wizard Privacy Policy', 'scudo' ); ?></h2>
        <p class="description"><?php esc_html_e( 'Compila i campi e genera automaticamente una pagina Privacy Policy conforme agli Artt. 13-14 del GDPR. Tutti i campi con * sono obbligatori per legge.', 'scudo' ); ?></p>
        ...
        <?php if ( ! empty( $data['generated_page_id'] ) && get_post( $data['generated_page_id'] ) ) : ?>
            <div class="notice notice-success inline" style="margin:16px 0;padding:12px;">
                <p>
                    <?php esc_html_e( 'Privacy policy generata:', 'scudo' ); ?>
                    <a href="<?php echo esc_url( get_permalink( $data['generated_page_id'] ) ); ?>" target="_blank"><?php esc_html_e( 'Visualizza', 'scudo' ); ?></a> |
                    <a href="<?php echo esc_url( get_edit_post_link( $data['generated_page_id'] ) ); ?>"><?php esc_html_e( 'Modifica', 'scudo' ); ?></a>
                </p>
            </div>
        <?php endif; ?>
        ...
        <div id="scudo-wizard" style="max-width:800px;">
            ...
            <div class="scudo-wizard-step" style="background:#fff;border:1px solid #c3c4c7;border-radius:4px;padding:20px;margin:16px 0;">
                <h3 style="margin-top:0;"><?php esc_html_e( '1. Titolare del trattamento *', 'scudo' ); ?></h3>
                <table class="form-table" style="margin:0;">
                    <tr><th><label><?php esc_html_e( 'Ragione sociale / Nome *', 'scudo' ); ?></label></th>
                        <td><input type="text" id="pp_controller_name" value="<?php echo esc_attr( $data['controller_name'] ); ?>" class="regular-text" required></td></tr>
                    <tr><th><label><?php esc_html_e( 'Indirizzo sede legale *', 'scudo' ); ?></label></th>
                        <td><input type="text" id="pp_controller_address" value="<?php echo esc_attr( $data['controller_address'] ); ?>" class="large-text"></td></tr>
                    <tr><th><label><?php esc_html_e( 'Email *', 'scudo' ); ?></label></th>
            ...
            <div class="scudo-wizard-step" style="background:#fff;border:1px solid #c3c4c7;border-radius:4px;padding:20px;margin:16px 0;">
                ...
                <div id="pp_dpo_fields" style="<?php echo $data['dpo_enabled'] ? '' : 'display:none;'; ?>">
                    <table class="form-table" style="margin:0;">
                        ...
                        <tr><th><label><?php esc_html_e( 'Email DPO', 'scudo' ); ?></label></th>
            ...
            <!-- Step 3: Finalità -->
            <div class="scudo-wizard-step" style="background:#fff;border:1px solid #c3c4c7;border-radius:4px;padding:20px;margin:16px 0;">
                <h3 style="margin-top:0;"><?php esc_html_e( '3. Finalità del trattamento *', 'scudo' ); ?></h3>
                <p class="description"><?php esc_html_e( 'Seleziona tutte le finalità per cui il tuo sito raccoglie dati personali.', 'scudo' ); ?></p>
                <?php
                $purpose_labels = [
                    'site_functionality' => __( 'Funzionamento del sito web', 'scudo' ),
                    'contact_forms'      => __( 'Gestione richieste di contatto', 'scudo' ),
                    'analytics'          => __( 'Analisi statistiche (Google Analytics, ecc.)', 'scudo' ),
                    'marketing'          => __( 'Marketing e pubblicità', 'scudo' ),
                    'newsletter'         => __( 'Invio newsletter', 'scudo' ),
                    'ecommerce'          => __( 'E-commerce / vendita online', 'scudo' ),
                    'user_accounts'      => __( 'Gestione account utente', 'scudo' ),
            ...
            <div class="scudo-wizard-step" style="background:#fff;border:1px solid #c3c4c7;border-radius:4px;padding:20px;margin:16px 0;">
                <h3 style="margin-top:0;"><?php esc_html_e( '4. Servizi di terze parti *', 'scudo' ); ?></h3>
                <p class="description"><?php esc_html_e( 'Seleziona i servizi esterni utilizzati dal tuo sito.', 'scudo' ); ?></p>
                <?php
                $service_labels = [
            ...
            <div class="scudo-wizard-step" style="background:#fff;border:1px solid #c3c4c7;border-radius:4px;padding:20px;margin:16px 0;">
                ...
                <div id="pp_transfers_fields" style="<?php echo $data['transfers_extra_eu'] ? '' : 'display:none;'; ?>">
                    ...
                    <label style="display:block;margin:4px 0;"><input type="radio" name="pp_transfers_basis" value="scc" <?php checked( $data['transfers_basis'], 'scc' ); ?>> <?php esc_html_e( 'Clausole contrattuali standard (SCC)', 'scudo' ); ?></label>
                    <label style="display:block;margin:4px 0;"><input type="radio" name="pp_transfers_basis" value="consent" <?php checked( $data['transfers_basis'], 'consent' ); ?>> <?php esc_html_e( 'Consenso esplicito dell\'interessato', 'scudo' ); ?></label>
                </div>
            </div>

            <!-- Step 6: Conservazione -->
            <div class="scudo-wizard-step" style="background:#fff;border:1px solid #c3c4c7;border-radius:4px;padding:20px;margin:16px 0;">
                <h3 style="margin-top:0;"><?php esc_html_e( '6. Periodi di conservazione *', 'scudo' ); ?></h3>
                <table class="form-table" style="margin:0;">
                    <tr><th><label><?php esc_html_e( 'Dati form di contatto', 'scudo' ); ?></label></th>
                        <td><input type="text" id="pp_ret_contact" value="<?php echo esc_attr( $data['retention_contact'] ); ?>" class="regular-text"></td></tr>
                    <tr><th><label><?php esc_html_e( 'Dati analitici', 'scudo' ); ?></label></th>
                        <td><input type="text" id="pp_ret_analytics" value="<?php echo esc_attr( $data['retention_analytics'] ); ?>" class="regular-text"></td></tr>
                    <tr><th><label><?php esc_html_e( 'Dati marketing/newsletter', 'scudo' ); ?></label></th>
                        <td><input type="text" id="pp_ret_marketing" value="<?php echo esc_attr( $data['retention_marketing'] ); ?>" class="regular-text"></td></tr>
                    <tr><th><label><?php esc_html_e( 'Dati e-commerce/fatturazione', 'scudo' ); ?></label></th>
                        <td><input type="text" id="pp_ret_ecommerce" value="<?php echo esc_attr( $data['retention_ecommerce'] ); ?>" class="regular-text"></td></tr>
                    <tr><th><label><?php esc_html_e( 'Dati account utente', 'scudo' ); ?></label></th>
            ...
            <p style="margin:20px 0;">
                <button type="button" class="button button-primary button-hero" id="scudo-generate-pp">
                    <?php echo ! empty( $data['generated_page_id'] ) && get_post( $data['generated_page_id'] )
                        ? esc_html__( 'Aggiorna Privacy Policy', 'scudo' )
                        : esc_html__( 'Genera Privacy Policy', 'scudo' ); ?>
                </button>
        ...
        (function() {
            ...
            document.getElementById('scudo-generate-pp').addEventListener('click', function() {
                ...
                btn.disabled = true;
                status.textContent = '<?php echo esc_js( __( 'Generazione in corso...', 'scudo' ) ); ?>';
                ...
                fetch(ajaxurl, { method: 'POST', body: fd, credentials: 'same-origin' })
                    ...
                    .then(function(res) {
                        btn.disabled = false;
                        if (res.success) {
                            status.innerHTML = '<?php echo esc_js( __( 'Privacy policy generata!', 'scudo' ) ); ?> <a href="' + res.data.url + '" target="_blank"><?php echo esc_js( __( 'Visualizza', 'scudo' ) ); ?></a>';
    ...
    public static function ajax_generate(): void {
        ...
        $page_data = [
            'post_title'   => __( 'Privacy Policy', 'scudo' ),
    ...
    private static function generate_content( array $d ): string {
        ...
        // 1. Titolare
        $c .= '<h2>' . __( '1. Titolare del trattamento', 'scudo' ) . '</h2>';
        $c .= '<p>' . __( 'Il titolare del trattamento dei dati personali è:', 'scudo' ) . '</p>';
        ...
        if ( $d['controller_phone'] ) $c .= '<br>' . __( 'Tel:', 'scudo' ) . ' ' . esc_html( $d['controller_phone'] );
        if ( $d['controller_pec'] ) $c .= '<br>PEC: ' . esc_html( $d['controller_pec'] );
        $c .= '</p>';

        // 2. DPO
        if ( $d['dpo_enabled'] ) {
            $c .= '<h2>' . __( '2. Responsabile della Protezione dei Dati (DPO)', 'scudo' ) . '</h2>';
        ...
        $c .= '<h2>' . __( '3. Finalità e basi giuridiche del trattamento', 'scudo' ) . '</h2>';
        $c .= '<p>' . __( 'I tuoi dati personali vengono trattati per le seguenti finalità:', 'scudo' ) . '</p>';

        $purpose_details = [
            'site_functionality' => [ __( 'Funzionamento del sito web', 'scudo' ), __( 'Legittimo interesse (Art. 6.1.f GDPR)', 'scudo' ), __( 'Garantire il corretto funzionamento tecnico del sito.', 'scudo' ) ],
            'contact_forms'      => [ __( 'Gestione richieste di contatto', 'scudo' ), __( 'Consenso (Art. 6.1.a GDPR) / Esecuzione di misure precontrattuali (Art. 6.1.b)', 'scudo' ), __( 'Rispondere alle richieste inviate tramite i moduli di contatto.', 'scudo' ) ],
            'analytics'          => [ __( 'Analisi statistiche', 'scudo' ), __( 'Consenso (Art. 6.1.a GDPR)', 'scudo' ), __( 'Raccogliere dati anonimi sull\'utilizzo del sito per migliorare i servizi offerti.', 'scudo' ) ],
            'marketing'          => [ __( 'Marketing e pubblicità', 'scudo' ), __( 'Consenso (Art. 6.1.a GDPR)', 'scudo' ), __( 'Mostrare annunci personalizzati e misurare l\'efficacia delle campagne pubblicitarie.', 'scudo' ) ],
            'newsletter'         => [ __( 'Invio newsletter', 'scudo' ), __( 'Consenso (Art. 6.1.a GDPR)', 'scudo' ), __( 'Inviare comunicazioni periodiche su novità, offerte e contenuti.', 'scudo' ) ],
            'ecommerce'          => [ __( 'E-commerce', 'scudo' ), __( 'Esecuzione del contratto (Art. 6.1.b GDPR) / Obbligo legale (Art. 6.1.c)', 'scudo' ), __( 'Gestire ordini, pagamenti, spedizioni e obblighi fiscali.', 'scudo' ) ],
            'user_accounts'      => [ __( 'Gestione account utente', 'scudo' ), __( 'Esecuzione del contratto (Art. 6.1.b GDPR)', 'scudo' ), __( 'Consentire la registrazione e la gestione dell\'area personale.', 'scudo' ) ],
            'comments'           => [ __( 'Commenti', 'scudo' ), __( 'Consenso (Art. 6.1.a GDPR)', 'scudo' ), __( 'Consentire la pubblicazione di commenti sul blog.', 'scudo' ) ],
        ];

        $c .= '<table style="width:100%;border-collapse:collapse;"><thead><tr><th style="border:1px solid #ddd;padding:8px;text-align:left;">' . __( 'Finalità', 'scudo' ) . '</th><th style="border:1px solid #ddd;padding:8px;text-align:left;">' . __( 'Base giuridica', 'scudo' ) . '</th><th style="border:1px solid #ddd;padding:8px;text-align:left;">' . __( 'Descrizione', 'scudo' ) . '</th></tr></thead><tbody>';
        ...
        // 4. Dati raccolti
        $c .= '<h2>' . __( '4. Tipologie di dati raccolti', 'scudo' ) . '</h2>';
        $c .= '<p>' . __( 'Il Sito può raccogliere le seguenti tipologie di dati personali:', 'scudo' ) . '</p>';
        $c .= '<ul>';
        $c .= '<li>' . __( '<strong>Dati di navigazione:</strong> indirizzo IP, tipo di browser, sistema operativo, pagine visitate, data e ora di accesso. Questi dati sono raccolti automaticamente dai sistemi informatici del Sito.', 'scudo' ) . '</li>';
        if ( ! empty( $d['purposes']['contact_forms'] ) ) $c .= '<li>' . __( '<strong>Dati forniti volontariamente:</strong> nome, email, telefono e altri dati inseriti nei moduli di contatto.', 'scudo' ) . '</li>';
        if ( ! empty( $d['purposes']['ecommerce'] ) ) $c .= '<li>' . __( '<strong>Dati di acquisto:</strong> indirizzo di fatturazione e spedizione, dati di pagamento (gestiti da fornitori terzi certificati PCI-DSS).', 'scudo' ) . '</li>';
        if ( ! empty( $d['purposes']['user_accounts'] ) ) $c .= '<li>' . __( '<strong>Dati di registrazione:</strong> nome utente, email, password (criptata).', 'scudo' ) . '</li>';
        $c .= '<li>' . __( '<strong>Cookie e tecnologie di tracciamento:</strong> per maggiori informazioni, consulta la nostra Cookie Policy.', 'scudo' ) . '</li>';
        $c .= '</ul>';

        // 5. Servizi terze parti
        $active_services = array_filter( $d['services'] );
        if ( ! empty( $active_services ) ) {
            $c .= '<h2>' . __( '5. Servizi di terze parti', 'scudo' ) . '</h2>';
            $c .= '<p>' . __( 'Il Sito utilizza i seguenti servizi di terze parti che possono raccogliere dati personali:', 'scudo' ) . '</p>';

            $service_details = [
                'google_analytics' => [ 'Google Analytics', 'Google LLC', __( 'Servizio di analisi statistica. Cookie utilizzati: _ga, _gid, _gat.', 'scudo' ), 'https://policies.google.com/privacy' ],
                'google_ads'       => [ 'Google Ads', 'Google LLC', __( 'Servizio di pubblicità. Può installare cookie di remarketing e conversione.', 'scudo' ), 'https://policies.google.com/privacy' ],
                'google_maps'      => [ 'Google Maps', 'Google LLC', __( 'Servizio di mappe interattive.', 'scudo' ), 'https://policies.google.com/privacy' ],
                'google_fonts'     => [ 'Google Fonts', 'Google LLC', __( 'Servizio di hosting font tipografici.', 'scudo' ), 'https://policies.google.com/privacy' ],
                'youtube'          => [ 'YouTube', 'Google LLC', __( 'Piattaforma di hosting video. Può installare cookie di profilazione.', 'scudo' ), 'https://policies.google.com/privacy' ],
                'facebook_pixel'   => [ 'Facebook Pixel', 'Meta Platforms Inc.', __( 'Strumento di tracciamento per pubblicità su Facebook/Instagram.', 'scudo' ), 'https://www.facebook.com/privacy/policy/' ],
                'mailchimp'        => [ 'Mailchimp', 'Intuit Inc.', __( 'Servizio di email marketing per l\'invio di newsletter.', 'scudo' ), 'https://www.intuit.com/privacy/statement/' ],
                'stripe'           => [ 'Stripe', 'Stripe Inc.', __( 'Servizio di elaborazione pagamenti.', 'scudo' ), 'https://stripe.com/privacy' ],
                'paypal'           => [ 'PayPal', 'PayPal Holdings Inc.', __( 'Servizio di elaborazione pagamenti.', 'scudo' ), 'https://www.paypal.com/privacy' ],
                'cloudflare'       => [ 'Cloudflare', 'Cloudflare Inc.', __( 'Servizio CDN e di sicurezza web.', 'scudo' ), 'https://www.cloudflare.com/privacypolicy/' ],
                'hotjar'           => [ 'Hotjar', 'Hotjar Ltd.', __( 'Servizio di analisi comportamentale (heatmap, registrazioni sessioni).', 'scudo' ), 'https://www.hotjar.com/privacy/' ],
            ];

            $c .= '<table style="width:100%;border-collapse:collapse;"><thead><tr><th style="border:1px solid #ddd;padding:8px;">' . __( 'Servizio', 'scudo' ) . '</th><th style="border:1px solid #ddd;padding:8px;">' . __( 'Fornitore', 'scudo' ) . '</th><th style="border:1px solid #ddd;padding:8px;">' . __( 'Descrizione', 'scudo' ) . '</th><th style="border:1px solid #ddd;padding:8px;">' . __( 'Privacy Policy', 'scudo' ) . '</th></tr></thead><tbody>';
        ...
        if ( $d['transfers_extra_eu'] ) {
            $c .= '<h2>' . __( '6. Trasferimento dei dati fuori dall\'UE', 'scudo' ) . '</h2>';
            $c .= '<p>' . __( 'Alcuni dei servizi di terze parti utilizzati comportano il trasferimento dei dati personali verso paesi situati al di fuori dello Spazio Economico Europeo (SEE).', 'scudo' ) . '</p>';
            $basis_texts = [
                'dpf'     => __( 'Tali trasferimenti avvengono sulla base del EU-US Data Privacy Framework (DPF), decisione di adeguatezza della Commissione Europea del 10 luglio 2023, verso aziende certificate sotto il DPF.', 'scudo' ),
                'scc'     => __( 'Tali trasferimenti avvengono sulla base delle Clausole Contrattuali Standard (SCC) approvate dalla Commissione Europea, che garantiscono un livello adeguato di protezione dei dati.', 'scudo' ),
                'consent' => __( 'Tali trasferimenti avvengono sulla base del tuo consenso esplicito, prestato ai sensi dell\'Art. 49(1)(a) del GDPR.', 'scudo' ),
            ];
            $c .= '<p>' . ( $basis_texts[ $d['transfers_basis'] ] ?? $basis_texts['dpf'] ) . '</p>';
        }

        // 7. Conservazione
        $c .= '<h2>' . __( '7. Periodo di conservazione dei dati', 'scudo' ) . '</h2>';
        $c .= '<p>' . __( 'I dati personali sono conservati per il tempo strettamente necessario al raggiungimento delle finalità per cui sono stati raccolti:', 'scudo' ) . '</p>';
        $c .= '<ul>';
        if ( ! empty( $d['purposes']['contact_forms'] ) ) $c .= '<li>' . __( 'Dati dei moduli di contatto:', 'scudo' ) . ' ' . esc_html( $d['retention_contact'] ) . '</li>';
        if ( ! empty( $d['purposes']['analytics'] ) ) $c .= '<li>' . __( 'Dati analitici:', 'scudo' ) . ' ' . esc_html( $d['retention_analytics'] ) . '</li>';
        if ( ! empty( $d['purposes']['marketing'] ) || ! empty( $d['purposes']['newsletter'] ) ) $c .= '<li>' . __( 'Dati marketing/newsletter:', 'scudo' ) . ' ' . esc_html( $d['retention_marketing'] ) . '</li>';
        if ( ! empty( $d['purposes']['ecommerce'] ) ) $c .= '<li>' . __( 'Dati e-commerce/fatturazione:', 'scudo' ) . ' ' . esc_html( $d['retention_ecommerce'] ) . '</li>';
        if ( ! empty( $d['purposes']['user_accounts'] ) ) $c .= '<li>' . __( 'Dati account utente:', 'scudo' ) . ' ' . esc_html( $d['retention_accounts'] ) . '</li>';
        $c .= '</ul>';

        // 8. Diritti
        $c .= '<h2>' . __( '8. I tuoi diritti', 'scudo' ) . '</h2>';
        $c .= '<p>' . __( 'Ai sensi degli Artt. 15-22 del GDPR, hai il diritto di:', 'scudo' ) . '</p>';
        $c .= '<ul>';
        $c .= '<li>' . __( '<strong>Accesso</strong> (Art. 15): ottenere conferma del trattamento dei tuoi dati e una copia degli stessi.', 'scudo' ) . '</li>';
        $c .= '<li>' . __( '<strong>Rettifica</strong> (Art. 16): ottenere la correzione dei dati inesatti o l\'integrazione di quelli incompleti.', 'scudo' ) . '</li>';
        $c .= '<li>' . __( '<strong>Cancellazione</strong> (Art. 17): ottenere la cancellazione dei tuoi dati ("diritto all\'oblio").', 'scudo' ) . '</li>';
        $c .= '<li>' . __( '<strong>Limitazione</strong> (Art. 18): ottenere la limitazione del trattamento in determinati casi.', 'scudo' ) . '</li>';
        $c .= '<li>' . __( '<strong>Portabilità</strong> (Art. 20): ricevere i tuoi dati in un formato strutturato e di uso comune.', 'scudo' ) . '</li>';
        ...
        // 9. Reclamo
        $c .= '<h2>' . __( '9. Diritto di reclamo', 'scudo' ) . '</h2>';
        $c .= '<p>' . __( 'Hai il diritto di proporre reclamo all\'autorità di controllo competente. In Italia, l\'autorità è il Garante per la Protezione dei Dati Personali:', 'scudo' ) . '</p>';
    ...
    public static function shortcode(): string {
        ...
        if ( empty( $data['controller_name'] ) ) {
            return '<p>' . __( 'La privacy policy non è ancora stata configurata. Vai in Impostazioni → Scudo per generarla.', 'scudo' ) . '</p>';
        }
